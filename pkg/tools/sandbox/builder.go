...
func (b *Builder) newNSMgrProxy(ctx context.Context, registryURL, registryProxyURL *url.URL) *EndpointEntry {
	...
	mgr := b.supplyNSMgrProxy(ctx,
		...
		nsmgrproxy.WithRegistryConnectOptions(
			registryconnect.WithClientDialOptions(DefaultDialOptions(b.generateTokenFunc)...),
		),
	)
...
func (b *Builder) SetupRegistryClients(ctx context.Context, node *Node, regUrl *url.URL) {
	var regCC grpc.ClientConnInterface
	if node.NSMgr != nil {
		regCC = b.dialContext(ctx, node.NSMgr.URL)
	} else {
		regCC = b.dialContext(ctx, regUrl)
	}

	node.ForwarderRegistryClient = client.NewNetworkServiceEndpointRegistryInterposeClient(ctx, regCC)
	node.EndpointRegistryClient = client.NewNetworkServiceEndpointRegistryClient(ctx, regCC)
	node.NSRegistryClient = client.NewNetworkServiceRegistryClient(regCC)
}
